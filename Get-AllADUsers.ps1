...
foreach ($Domain in $Domains) {
    ...
    $Users = Get-ADUser -Filter $Filter -Server (Get-ADDomainController -Discover -DomainName $Domain)
    ...
    foreach ($User in $Users) {
        ...
        $UserHashtable[$User.SamAccountName] = "$($User.SamAccountName),$($User.GivenName),$($User.Surname),$($User.UserPrincipalName)"
    }
}
# write our header
"AccountName,First,Last,EMail" | Out-File -FilePath $OutFile -Force
